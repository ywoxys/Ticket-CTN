    ...
    const handleApproval = async (id: string, status: 'aprovado' | 'rejeitado') => {
        ...
        try {
            ...
            if (status === 'aprovado' && solicitacao?.ticket) {
                ...
                let webhookUrl = `http://195.200.5.252:5678/webhook-test/trello-ctn?atendente=${encodeURIComponent(atendenteNome)}&matricula=${encodeURIComponent(ticket.matricula)}&nome=${encodeURIComponent(ticket.nome)}&valor=${ticket.valor}&qtd=${ticket.qtd_mensalidades}&telefone=${ticket.telefone}&categoria=${ticket.categoria}`;
